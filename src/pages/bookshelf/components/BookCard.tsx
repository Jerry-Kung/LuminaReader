...
import { formatFileSize, formatTimeAgo, generateCoverColor, generateCoverPattern } from '@/mocks/bookshelf';
  ...
  return (
    <div className="group relative flex flex-col">
      ...
      <div className="mt-2.5 px-0.5">
        ...
        >
          <h3 className="text-sm font-medium text-stone-700 truncate leading-tight" title={book.title}>
            {book.title}
          </h3>
        </button>
        <div className="flex items-center gap-2 mt-1">
          <span className="text-xs text-stone-400">{formatFileSize(book.file_size)}</span>
          <span className="text-xs text-stone-300">&middot;</span>
          <span className="text-xs text-stone-400">{formatTimeAgo(book.updated_at)}</span>
        </div>
      </div>
    </div>
  );
